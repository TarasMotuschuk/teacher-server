...
public sealed class MacOsRawWindowVideoSource : IVideoSource, IDisposable
{
    private readonly Vp8EncodedRawVideoSource _inner;
    private readonly MacOsWindowCaptureProducer _producer;
    private readonly long _windowId;
    private readonly int _captureFps;
    private bool _started;

    public MacOsRawWindowVideoSource(long windowId, string? windowTitle, int captureFps)
    {
        _ = windowTitle;
        _windowId = windowId;
        ...
        _producer = new MacOsWindowCaptureProducer();
    }
    ...
    public async Task StartVideo()
    {
        ...
        {
            _started = true;
            _producer.Start(_windowId, _captureFps, _inner.ExternalVideoSourceRawSample);
        }
    }
